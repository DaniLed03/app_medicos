        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-xl font-bold text-gray-900 uppercase">Lista de Secretarias</h1>
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-4">
                            <!-- Contenedor de Total Secretarias -->
                            <div class="bg-white p-4 shadow-2xl rounded-md flex items-center space-x-4">
                                ...
                                <div class="text-center">
                                    <h2 class="text-lg font-bold">Total Secretarias: {{ $totalSecretarias }}</h2>
                                </div>
                            </div>
                    ...
                    <div class="overflow-x-auto bg-white dark:bg-neutral-700">
                        <table id="secretariasTable" class="display nowrap" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Nombres</th>
                            ...
                            <tbody>
                                @foreach($secretarias as $secretaria)
                                    <tr>
                                        <td>{{ $secretaria->nombres }} {{ $secretaria->apepat }} {{ $secretaria->apemat }}</td>
                                        <td>{{ \Carbon\Carbon::parse($secretaria->fechanac)->format('j M, Y') }}</td>
                                        <td>{{ $secretaria->sexo }}</td>
                                        <td>{{ $secretaria->telefono }}</td>
                                        <td>{{ $secretaria->email }}</td>
                                        <td>
                                            <a href="#" data-id="{{ $secretaria->id }}" class="text-blue-500 hover:underline editarSecretaria">Editar</a>
                                            <form action="{{ route('secretarias.destroy', $secretaria->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-screen">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mt-2">
                        @include('medico.secretaria.agregarSecretaria')
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-center min-h-screen">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="border-t border-gray-200 mt-4"></div>
                    <div class="mt-2" id="formEditarSecretaria">
...
<script>
    $(document).ready(function() {
        $('#secretariasTable').DataTable({
            "language": {
                "decimal": "",
                "emptyTable": "No hay secretarias registradas",
        ...
        document.querySelectorAll('.editarSecretaria').forEach(button => {
            button.addEventListener('click', function(event) {
                event.preventDefault();
                const id = this.getAttribute('data-id');
                fetch(`/secretarias/${id}/edit`)
                    .then(response => response.text())
                    .then(html => {
                        document.getElementById('formEditarSecretaria').innerHTML = html;
